...
export class WecomAppChannel extends BaseChannel {
  ...
  async sendMessage(
    ...
    const tokenResponse = await fetch(
      `http://124.220.38.74/wecom-proxy/cgi-bin/gettoken?corpid=${corpId}&corpsecret=${secret}`
    )
    ...
    const response = await fetch(
      `http://124.220.38.74/wecom-proxy/cgi-bin/message/send?access_token=${tokenData.access_token}`,
      {
        method: 'POST',
        headers: {
